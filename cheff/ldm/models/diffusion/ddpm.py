...
class LatentDiffusion(DDPM):
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False, att_mask=None):
        ...
        if hasattr(self, "split_input_params"):
            ...
            output_list = [self.model(z_list[i], t, **cond_list[i], att_mask=att_mask) for i in range(z.shape[-1])]
    ...
    def p_mean_variance(
        ...
        t_in = t
        model_out = self.apply_model(x, t_in, c, return_ids=return_codebook_ids, att_mask=att_mask)
    ...
    def make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0.0, verbose=True):
        self.ddpm_num_timesteps = self.num_timesteps
    ...
    def d_p_sample_loop(
        ...
        self.make_schedule(ddim_num_steps=timesteps, ddim_eta=1, verbose=False)
        ...
        for i, step in enumerate(iterator):
            ...
            # if i % log_every_t == 0: # or index == total_steps - 1:
            #     intermediates["x_inter"].append(img)
            #     intermediates["pred_x0"].append(pred_x0)

        return img, pred_x0, intermediates
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        super().__init__()
        self.diffusion_model: UNetModel = instantiate_from_config(diff_model_config)
        self.conditioning_key = conditioning_key
        assert self.conditioning_key in [None, "concat", "crossattn", "hybrid", "adm"]

    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, att_mask=None, *args, **kwargs):
        if self.conditioning_key is None:
            out = self.diffusion_model(x, t, mask=att_mask)
        elif self.conditioning_key == "concat":  # Using this !!
            xc = torch.cat([x] + c_concat, dim=1)
            out = self.diffusion_model(xc, t, mask=att_mask)
        elif self.conditioning_key == "crossattn":
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, context=cc, mask=att_mask)
        elif self.conditioning_key == "hybrid":  # TODO try this out with class labels
            xc = torch.cat([x] + c_concat, dim=1)
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(xc, t, context=cc, mask=att_mask)
        elif self.conditioning_key == "adm":
            cc = c_crossattn[0]
            out = self.diffusion_model(x, t, y=cc, mask=att_mask)
        else:
            raise NotImplementedError()
