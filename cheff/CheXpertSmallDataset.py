...
class ChexpertSmall(Dataset):
    ...
    def __init__(self, root, mode="train", transform=None, data_filter=None, mini_data=None):
        ...
        if mode == "test":
            ...
            self.data[self.attr_names] = pd.DataFrame(
                np.zeros((len(self.data), len(self.attr_names)))
            )  # attr is vector of 0s under test
        else:
            ...
            if mode == "vis":
                ...
                for attr in self.attr_names:  # 1 only condition category
                    idxs.append(
                        self.data.loc[
                            (self.data[attr] == 1) & (self.data[self.attr_names].sum(1) == 1), self.attr_names
                        ]
                        .head(3)
                        .index.tolist()
                    )
                idxs.append(
                    self.data.loc[self.data[self.attr_names].sum(1) == 0, self.attr_names].head(3).index.tolist()
                )  # no findings category
                idxs.append(
                    self.data.loc[self.data[self.attr_names].sum(1) == 2, self.attr_names].head(3).index.tolist()
    ...
    def _maybe_download_and_extract(self):
        ...
        print("PATHHH", os.path.join(self.root, self.dir_name))
        if not os.path.exists(os.path.join(self.root, self.dir_name)):
            ...
            if not os.path.exists(os.path.join(self.root, os.path.basename(self.url))):
                ...
                def _progress(count, block_size, total_size):
                    sys.stdout.write(
                        "\r>> Downloading %s %.1f%%" % (fpath, float(count * block_size) / float(total_size) * 100.0)
                    )
                    sys.stdout.flush()
