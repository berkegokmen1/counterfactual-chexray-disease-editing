...
from chexzero.components.mlp import MLP
from chexzero.img_encoder import ImageEncoderOutput
...
class ChexzeroImageEncoderConfig(BaseModelConfig):
    model_path: str = os.path.expanduser(
        "~/models/third_party/chexzero/CheXzero_Models/best_64_5e-05_original_22000_0.864.pt"
    )
...
class ChexzeroImageEncoder(BaseModel):
    ...
    def forward(self, x: Tensor, **kwargs) -> ImageEncoderOutput:
        ...
        input_resolution = H
        assert (
            self.backbone.input_resolution == input_resolution
        ), f"Input resolution of backbone ({self.backbone.input_resolution}) does not match input resolution of image ({input_resolution})"
        ...
        return ImageEncoderOutput(
            patch_features=projected_patch_features, pos_embeddings=pos_embeddings, global_features=cls_features
        )
